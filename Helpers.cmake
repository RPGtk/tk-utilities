...
function(create_target EXECUTABLE)
    ...
    foreach(source ${args_RAW_SOURCES})
        set(TRUE_SOURCE "${SOURCE_DIR}/${source}.c")
        list(APPEND SOURCES ${TRUE_SOURCE})
        get_source_file_property(CURRENT_CDS ${TRUE_SOURCE} COMPILE_DEFINITIONS)
        set_source_files_properties(${TRUE_SOURCE} PROPERTIES COMPILE_DEFINITIONS "${CURRENT_CDS};FILENAME=\"${source}.c\"")
    endforeach()
...
function(link_external NAME)
    ...
    if(NOT args_COMPONENTS)
        ...
        foreach(component ${args_COMPONENTS})
            list(APPEND LINK_TARGETS ${${NAME}_${component}_LIBRARY})
            message(STATUS ${${NAME}_${component}_LIBRARY})
        endforeach()
